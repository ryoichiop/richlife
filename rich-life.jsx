          ...
          {reviewFiltered.length===0?(<div style={{textAlign:"center",padding:"80px 0"}}><div style={{fontSize:48,marginBottom:16,opacity:0.15}}>✓</div><div style={{fontSize:18,fontWeight:600,marginBottom:8,color:"#2A9D8F"}}>{reviewShowAll?"Nenhuma transação encontrada.":"Tudo categorizado!"}</div><div style={{fontSize:13,color:C.t3}}>{reviewShowAll?"Use os filtros ou faça upload de novos documentos.":"Nenhuma transação pendente."}</div>{!reviewShowAll&&<button onClick={()=>setView("upload")} style={{...S.btn,background:C.greenBg,color:"#2A9D8F",marginTop:16}}>Upload</button>}</div>):(<div>
            ...
            <div style={{...S.card,padding:0,overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>{reviewFiltered.slice(0,200).map(t=>(<tr key={t.id} style={{borderBottom:"1px solid "+C.borderLight,background:selectedTxns.has(t.id)?"rgba(108,99,255,0.06)":t.category?C.card:C.redBg}}>
                  <td style={{padding:"4px 8px",textAlign:"center"}}><input type="checkbox" checked={selectedTxns.has(t.id)} onChange={e=>{setSelectedTxns(prev=>{const n=new Set(prev);if(e.target.checked)n.add(t.id);else n.delete(t.id);return n;});}} style={{cursor:"pointer"}}/></td>
                  <td style={{padding:"8px 14px",color:C.t3,fontFamily:"'Space Mono',monospace",fontSize:11,whiteSpace:"nowrap"}}>{t.date?t.date.toLocaleDateString("pt-BR"):"—"}</td>
                  <td style={{padding:"8px 14px",maxWidth:280,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.description}>{t.description}</td>
                  <td style={{padding:"8px 14px",textAlign:"right",fontFamily:"'Space Mono',monospace",fontWeight:600}}>{t.currency&&t.currency!=="BRL"?(<div><div>{fmtCurrency(t.originalValue,t.currency)}</div><div style={{fontSize:10,color:C.t4,fontWeight:400}}>{fmt(t.brlValue)}</div></div>):fmt(t.brlValue!=null?t.brlValue:t.value)}</td>
                  <td style={{padding:"8px 14px",fontSize:11,fontFamily:"'Space Mono',monospace",color:C.t3}}>{t.monthKey||"—"}</td>
                  ...
                  <td style={{padding:"8px 14px",fontSize:10,color:C.t4,maxWidth:150,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={t.source}>{t.source}</td>
